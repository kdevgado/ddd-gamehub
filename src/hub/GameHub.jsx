...
function GameCard({ game, onOpenRoom, spotlight }) {
  ...
  return (
    <Tag className={`lounge-game ${game.featured ? "game-featured" : ""} ${spotlight ? "is-picked" : ""}`} data-scroll-scene={spotlight ? undefined : ""} data-scroll-reveal={spotlight ? undefined : ""} {...action} aria-label={`Play ${game.name}`}>
...
export default function GameHub({ onOpenRoom }) {
  ...
  return (
    <div className="lounge" ref={rootRef}>
      <div className="lounge-scroll-track" aria-hidden="true"><i /><span>✳</span></div>
      ...
      <main>
        ...
        <section className="lounge-collection lounge-width" id="collection" aria-labelledby="collection-title">
          <div className="collection-heading" data-scroll-reveal><div><p className="lounge-eyebrow">The collection <span className="collection-count">07</span></p><h2 id="collection-title">Find your kind of fun<span>.</span></h2></div><p>Same couch or miles apart.<br />There’s a game for that.</p></div>
      ...
      <footer className="lounge-footer lounge-width" data-scroll-reveal><a className="footer-wordmark" href="/">ddd<span> / game hub</span></a><span>Made for the moments between everything else.</span><a href="#lounge-title">Back to the good stuff <Icon name="diagonal" size={14} /></a></footer>
